...
router.get('/', async (req,res,next) => {
    Delivery.find()
             .sort({id:1})
             .then(delievery => {
                return res.render('deliveries/index', {title:"Delievery Schedules",delieveryData:delievery});
             })
             .catch(err =>{
...
router.get('/details/:deliveryIdNumber', async (req,res,next)=> {
    Delivery.find({deliveryId: req.params.deliveryIdNumber})
             .then(deli => {
                ...
                Material.find({deliveryID: deli[0].deliveryId})
                        .then(data => {
                            return res.render('deliveries/details',{title:"Details",delivery: deli[0], materialData: data})
                        })
             })
             .catch(err => {
                return res.status(500).json(err);
...
router.get('/map/:cityName', async (req,res,next) =>{
    ...
    fetch(`https://api.weatherapi.com/v1/current.json?key=${process.env.WEATHER_API_KEY}&q=${city},ON&api=no`)
                .then(weatherAPI => weatherAPI.json())
                .then(data => {
                    res.render('deliveries/map', {title:"Map Location",mapData: data});
                })
                .catch(err => {
...
router.get('/add',(req,res,next) =>{
    Project.find()
           .then(projects => {
                return res.render("deliveries/add", {title:"Add Delivery",projectData:projects});
           })
           .catch(err =>{
                return res.status(500).json(err);
           })
...
router.get("/edit/:_id", async (req,res,next) =>{
    ...
    res.render("deliveries/edit",{title: "Edit Current Delivery", deliveryData: data }) 
});
